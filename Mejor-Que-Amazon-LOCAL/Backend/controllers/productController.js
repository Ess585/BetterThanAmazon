...
router.post('/create', async (req, res) => {
    ...
    try {
        let errors = [];
        if (title.length < 3 || title.length > 50) errors.push('El título debe tener al menos 3 caracteres y un máximo de 50 caracteres ');
        if (isNaN(Number(price))) errors.push('El precio debe ser un número ');
        if (description.length < 10 || description.length > 1000) errors.push('La descripción debe tener al menos 10 caracteres y un máximo de 1000 caracteres ');
        if (/^[A-Za-z]+$/.test(city) == false) errors.push('La ciudad debe contener solo letras en español ')
        if (!image.includes('image')) errors.push('El archivo cargado debe ser una imagen ');
        if (!category) errors.push('La categoria es requerida ');
...
router.patch('/edit/:id', isAuth, async (req, res) => {
    ...
    try {
        ...
        if (user._id.toString() !== product.seller.toString()) {
            errors.push('¡No tienes permiso para realizar esta acción! ')
        }
        ...
        if (/^[A-Za-z]+$/.test(city) == false) errors.push('La ciudad debe contener solo letras en español ')
        if (req.body.image) {
            if (!req.body.image.includes('image')) errors.push('El archivo cargado debe ser una imagen ');
        }
        if (!category || category == "Choose...") errors.push('La categoria es requerida; ');
...
router.get('/enable/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({ msg: "Activado" });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
});
...
router.get('/archive/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({ msg: "Archivado" });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
});
...
router.get('/wish/:id', async (req, res) => {
    try {
        ...
        if (!user.wishedProducts.includes(req.params.id)) {
            ...
            res.status(200).json({ msg: "Deseado" });
        } else {
            ...
            res.status(200).json({ msg: "No deseado" });
        }
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
